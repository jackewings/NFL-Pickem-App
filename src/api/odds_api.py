        ...
        try:
            ...
            for game in data:
                if game.get('completed', False) and 'scores' in game:
                    try:
                        # Extract scores from the array format
                        home_score = next(
                            int(s['score']) for s in game['scores'] 
                        ...
                        logger.warning(f"Could not process scores for game: {game}")
                        continue
            
            logger.info(f"Processed {len(processed_data)} completed games")
